<template>
  <div>
    <a-card :bordered="false" :bodyStyle="{ padding: '0 32px', height: '100%' }" :headStyle="{'border-bottom': 'none'}">
      <div slot="title">
        <a-button type="primary" icon="plus" @click="$router.push('/server/add')">添加服务器</a-button>
      </div>
      <div slot="extra">
      ...
      >
        ...
        <template slot="action" slot-scope="text, record">
          <a-dropdown>
            ...
            <a-menu slot="overlay">
              <a-menu-item key="edit" @click="$router.push(`/server/${record.id}`)">编辑</a-menu-item>
...
import { getServers, delServer } from '@/api/server'
