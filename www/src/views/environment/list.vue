<template>
  <div>
    <a-card :bordered="false" :bodyStyle="{ padding: '0 32px', height: '100%' }" :headStyle="{'border-bottom': 'none'}">
      <div slot="title">
        <a-button type="primary" icon="plus" @click="$router.push('/environment/add')">添加环境</a-button>
      </div>
      <div slot="extra">
      ...
      >
        ...
        <template slot="action" slot-scope="text, record">
          <a-dropdown>
            ...
            <a-menu slot="overlay">
              <a-menu-item key="edit" @click="$router.push(`/environment/${record.id}`)">编辑</a-menu-item>
...
import { getEnvironments, delEnvironment } from '@/api/environment'
...
export default {
  ...
  methods: {
    ...
    loadDatas () {
      ...
      getEnvironments(params).then(res => {
    ...
    delItem (item) {
      ...
      this.$confirm({
        ...
        cancelText: '取消',
        onOk () {
          delEnvironment({ id: item.id }).then(res => {
            if (res.code === 0) {
